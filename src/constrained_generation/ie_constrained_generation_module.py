...
class IEConstrainedGeneration(ConstrainedGenerationModule):
        ...
        if identifier is not None:
            ...
            if os.path.isdir(path_to_trie_cache):
                if not override:
                    ...
                    entity_trie = Trie.load(os.path.join(path_to_trie_cache, f"entity_trie.pickle"))
                    relation_trie = Trie.load(os.path.join(path_to_trie_cache, "relation_trie.pickle"))
            ...
            path_to_trie_cache = None  # to prevent asserts failing in the helper functions
    ...
    def from_string_iterables(
        ...
        entity_trie = utils.get_trie_from_strings(entity_names, tokenizer=model.tokenizer)
        relation_trie = utils.get_trie_from_strings(relation_names, tokenizer=model.tokenizer)
